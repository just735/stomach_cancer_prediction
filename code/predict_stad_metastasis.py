"""
TCGA-STAD胃癌转移预测模型
参考 Prediction model of distant metastasis of stomach cancer.py 的写法
"""
...
DATA_DIR = BASE_DIR / "data" / "STAD"
OUTPUT_DIR = BASE_DIR / "output" / "STAD"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

print("=" * 60)
print("TCGA-STAD胃癌转移预测模型")
print("=" * 60)

# 读取预处理后的数据文件
data_path = DATA_DIR / "stad_processed.csv"
if not data_path.exists():
    print(f"\n错误: 数据文件不存在: {data_path}")
    print("请先运行 code/process_stad_data.py 生成处理后的数据")
...
if y.dtype == object:
    try:
        ...
        y = y.map(lambda v: 1 if str(v).lower() in ("1", "metastasis", "yes", "positive", "true", "m1") else 0)
...
# 特征选择（如果特征数较多）
k_select = min(100, n_features)  # 最多选择100个特征
if n_features > 10:
    print(f"\n进行特征选择: 从 {n_features} 个特征中选择前 {k_select} 个...")
    ...
    try:
        ...
        if len(unique_labels) < 2:
            ...
            if len(y_bin) == len(probs):
                ...
                plt.title("ROC curve - STAD Metastasis Prediction")
