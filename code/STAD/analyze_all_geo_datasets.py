...
print("=" * 100)
print("TCGA-STAD 和 GEO数据集全面对比分析")
print("=" * 100)
print()
...
if TCGA_PROCESSED.exists():
    ...
    print(f"\n4. 年龄统计:")
    if 'age_at_initial_pathologic_diagnosis' in tcga_data.columns:
        ages = tcga_data['age_at_initial_pathologic_diagnosis'].dropna()
        if len(ages) > 0:
    ...
    print(f"\n5. 性别分布:")
    if 'gender' in tcga_data.columns:
        gender_counts = tcga_data['gender'].value_counts()
        for gender, count in gender_counts.items():
            print(f"   - {gender}: {count} ({count/len(tcga_data)*100:.1f}%)")
    
    print(f"\n6. 肿瘤分期分布:")
    if 'ajcc_pathologic_stage' in tcga_data.columns:
        stage_counts = tcga_data['ajcc_pathologic_stage'].value_counts()
        for stage, count in stage_counts.items():
            print(f"   - {stage}: {count} ({count/len(tcga_data)*100:.1f}%)")
...
if GSE62254_PATH.exists():
    ...
    print(f"   - 样本数: {len(geo_data.get('!Sample_geo_accession', []))}")
    print(f"   - 数据来源: GEO (Gene Expression Omnibus)")
    
    print(f"\n2. 样本特征:")
    sample_chars = geo_data.get('!Sample_characteristics_ch1', [])
    if len(sample_chars) > 0:
        print(f"   - 发现 {len(set([c.split(':')[0].strip() for c in sample_chars if ':' in c]))} 个临床特征")
        for i, char in enumerate(sample_chars[:3]):
            if ':' in char:
                feature = char.split(':')[0].strip()
                print(f"   - {feature}: {char.split(':')[1].strip()}")
        print(f"   - ... (共 {len(sample_chars)} 个样本)")
...
# 样本数量
tcga_samples = len(tcga_data) if TCGA_PROCESSED.exists() else 0
gse62254_samples = len(geo_data.get('!Sample_geo_accession', [])) if GSE62254_PATH.exists() else 0
gse26901_samples = len(gse26901_data) if GSE26901_PATH.exists() else 0
gse84437_samples = len(geo_data.get('!Sample_geo_accession', [])) if GSE84437_PATH.exists() else 0
